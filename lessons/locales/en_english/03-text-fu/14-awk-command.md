...
Three things happen there that are unlike the shell. awk variables need no declaration, an unset one starts at zero, and <b>+=</b> means "add this to what is already there". So sum accumulates down the file, and END prints it once at the bottom. Note also that awk's own variables have no dollar sign: sum is a variable, while $3 is a field.
